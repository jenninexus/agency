...
| **Blog Include** | `public_html/includes/blog-posts.php` | `jn_render_recommended_posts()` + `jn_render_blog_cards()` + `jn_blog_post_image_src()` helpers. |
| **Blog Posts JSON** | `public_html/resources/playlists/blog-posts.json` | Blog post metadata: slug, title, date, category, image, tags. |
| **Blog Tag Protocol** | `storage/docs/PROTOCOL.md#tag-system` | Tag badge pattern: `<a href="/tags?filters=slug">` — NEVER `<span>` or `<button>`. |
| **Tag Definitions** | `public_html/resources/playlists/tags.json` | Canonical tag registry (id, slug, name, category). |
| **Content Tags** | `public_html/resources/playlists/content_tags.json` | Content-to-tag mappings (auto-generated by build scripts). |
| **Tag Filter JS** | `src/assets/js/tag-filter-api.js` | Central tag filtering state (bundled in core.js). |
| **Tag System JS** | `src/assets/js/tag-system.js` | Offcanvas tag panel UI + filtering (site-wide). |
| **Tag Page** | `public_html/tags.php` | Tag browser page — reads `?filters=` query param. |
...
**Inline blog renderers** (pages that render blog cards via JS) must use the same anchor pattern. Pages with inline blog rendering: `diy.php`, `gaming.php`, `youtube.php`, `ai.php`.
...
- **Galleries / WebP:** Follow `storage/docs/BLOG-SYSTEM.md` §4.4 — convert sources → WebP into `resources/images/blog/{slug}/`, wire via `.blog-gallery` (copy from `_template.php`). Keep `README-IMAGES.txt` in that folder. Theme colors only via tokens (`blog-theme.css` / GraphViz); no white backgrounds.
...
| `senior-cat-care.php` | ✅ PASS | 2026-05-25 | Senior pet care post; tags use `../tags.php?filters=` (corrected from root-relative on creation) |
