...
| **Cluster SSOT** | `[cluster]/README.md` or project override | Cross-project blog protocol rules (local). |
| **Blog Include** | `[blog-posts.php]` | Recommended-posts + blog-card helpers. |
| **Blog Posts JSON** | `[blog-posts.json]` | Blog post metadata: slug, title, date, category, image, tags. |
| **Blog Tag Protocol** | `storage/docs/PROTOCOL.md#tag-system` | Tag badge pattern: `<a href="/tags?filters=slug">` — NEVER `<span>` or `<button>`. |
| **Tag Definitions** | `[tags.json]` | Canonical tag registry (id, slug, name, category). |
| **Content Tags** | `[content_tags.json]` | Content-to-tag mappings (auto-generated by build scripts). |
| **Tag Filter JS** | `[tag-filter-api.js]` | Central tag filtering state. |
| **Tag System JS** | `[tag-system.js]` | Offcanvas tag panel UI + filtering. |
| **Tag Page** | `[tags.php]` | Tag browser page — reads `?filters=` query param. |
| **Images Folder** | `resources/images/agents/bloggie/` | Generated character images. |
...
### Discord chatVoice
Warm editor energy. Sample: *"Love the draft — add Topics: anchors and it reads studio-ready."* Runtime catalogue → consuming bot `agency-profiles.json`.
